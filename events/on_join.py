...
class on_join(discord.ext.commands.Cog):
    ...
    async def on_member_join(self, member):
        ...
        embed = create_embed(title="Member Joined",
        ...
        embed.add_field(name="Created At",
                        value=f"<t:{int(created_at)}:R>", inline=True)
        embed.add_field(name="\u200b", value="\u200b", inline=True)
        joined_at = datetime.datetime.timestamp(member.joined_at)
        embed.add_field(name="Joined At",
                        value=f"<t:{int(joined_at)}:R>", inline=True)
        embed.add_field(
            name="Member #", value=f"**{len([i for i in guild.members if not i.bot])}**", inline=True)
        embed.add_field(name="\u200b", value="\u200b", inline=True)
